...
| `POST /api/sessions/visitor` | tout staff | `{ visitorName, visitorPhone, paymentMethod }` | `201 { session }` |
...
```
`session-ineligible` → HTTP `422` avec `{ code, reason }` dans le corps, pour que le frontend puisse afficher exactement le même message que `IneligibilityNotice` affiche déjà (mêmes trois raisons).
