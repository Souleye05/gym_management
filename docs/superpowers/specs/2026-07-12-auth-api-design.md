...
- `LoginAttempt` alimente deux mécanismes de rate limiting distincts, distingués par la colonne `kind` (`AttemptKind`) : `LOGIN` (`identifier` = email tenté, échecs uniquement, staff) et `OTP_REQUEST` (`identifier` = numéro de téléphone, toutes tentatives confondues, endpoint `client/request-otp`). `OtpCode.attempts` reste un mécanisme séparé : il limite les mauvaises tentatives de code contre un OTP déjà émis (`client/verify-otp`), pas la fréquence des demandes de nouveaux codes.
...
| Rate limiting demandes OTP (`client/request-otp`) | Réel mais basique (compteur PostgreSQL par numéro de téléphone, `LoginAttempt.kind = OTP_REQUEST` ; 5 demandes / 15 min ; pas de dimension IP) |
| Rate limiting tentatives OTP (`client/verify-otp`) | Réel (compteur `OtpCode.attempts`, 5 tentatives max par code) |
