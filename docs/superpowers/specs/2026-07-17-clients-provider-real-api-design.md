...
export interface ClientRepository {
  ...
  /** total = le nombre total de clients actifs en base, indépendant de `limit` (vraie pagination). */
...
export interface ClientService {
  ...
  // query non-vide → délègue à repository.search(query), résultat { clients } SANS total —
  // search() n'a pas de vraie pagination, clients.length en ferait un champ trompeur
  // (donnerait l'illusion d'un total serveur indépendant du nombre de résultats renvoyés).
  // L'absence du champ, pas une valeur dérivée, est le signal qu'aucune pagination ne s'applique.
  // query absente/vide → délègue à repository.listActive(pagination ?? { page: 1, limit: DEFAULT_LIST_ACTIVE_LIMIT }),
  // résultat { clients, total } avec un total réel (COUNT indépendant de limit).
}
```

`GET /api/clients?page=&limit=` (nouveaux params optionnels, `q`/`phone`/`cardNumber` inchangés) → `{ clients: Client[], total?: number }` dans l'enveloppe `{ success, data, message, errors }` existante — `total` présent uniquement pour le chemin liste paginée, absent pour le chemin recherche/`phone`/`cardNumber`. `Client.email` est déjà `string | null` côté backend — aucun changement de forme nécessaire côté domaine ; c'est uniquement le frontend qui rattrape son retard.
...
**Invalidation du cache après mutation** : chaque mutation (`addClient`/`updateClient`/`deactivateClient`) invalide la ou les `queryKey` `['clients', ...]` actives dans son `onSuccess` (`queryClient.invalidateQueries({ queryKey: ['clients'] })` — invalide toutes les variantes paginées/recherche en une fois, plus simple et sûr qu'un ciblage fin vu le faible volume de requêtes en jeu). Ce détail relève du plan d'implémentation frontend, pas de ce document — noté ici pour qu'il ne soit pas oublié en aval.
...
- Optimistic updates sur les mutations (mise à jour immédiate du cache avant confirmation serveur, rollback si erreur) — bonus possible pour le plan frontend, pas requis pour ce chantier ; l'invalidation de cache post-`onSuccess` (voir section Contrat) suffit pour une UX correcte sans complexité de rollback.
